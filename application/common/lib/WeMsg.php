...
class WeMsg extends Controller
{
    ...
    protected $get_token_createUrl='https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%s&secret=%s';#获取access_token
    protected $temp_send_createUrl='https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=%s';#订阅消息发送模板
    protected $temp_list_createUrl='https://api.weixin.qq.com/cgi-bin/wxopen/template/list';#获取模板列表
    ...
    public function getToken(){
        if($this->wechat->access_expires<time()){
            $result=httpRequest(sprintf($this->get_token_createUrl,$this->app_id,$this->app_secret),'get');
    ...
    public function sendTempletMsg($type,$param){
        ...
        $result=httpRequest(sprintf($this->temp_send_createUrl,$this->getToken()),'post',json_encode($params));
        $result=json_decode($result,true);
        if($result['errcode']!==0){
    ...
    public function getTempletList(){
        ...
        $result=httpRequest($this->temp_list_createUrl,'post',$params);
        var_dump($result);
    }
}