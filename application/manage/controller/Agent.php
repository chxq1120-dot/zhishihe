    ...
    public function payment($ids = 0)
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                Db::startTrans();
                try {
                    if (empty($params['wechat_pay'])) {
                        $params['wechat_pay'] = 0;
                    }
                    if (empty($params['card_pay'])) {
                        $params['card_pay'] = 0;
                    }
                    $siteModel = new AdminSite();
                    if (!empty($params['id'])) {
        ...
        if ($ids) {
            if ($this->request->isAjax()) {
                try {
                    ...
                    list($result, $info) = AdminBill::money($data['mode'], 5, $data['money'], $ids, $data['remark'], 0);
                    if (!$result) {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    #是否同步推广资源
                    $auto_spread=intval(config('setting.agent_auto_spread'));
                    if($auto_spread==2){
                        $this->model->autoSpread($agents->id);
                    }
                    Db::commit();
                } catch (ValidateException $e) {
                    Db::rollback();
