...
{
    protected $searchFields = 'user.mobile,user.username,user.realname,user.nickname,invite.id,invite.nickname';
    ...
    {
        $min_price = config('setting.agent_min_svip');
        if (request()->isAjax()) {
            $data = input('post.');
            ...
            if ($this->auth->group_id == 2) {
                if ($data['price'] < $min_price) {
                    return callback(404, '会员套餐价格不能低于' . $min_price . '元');
                }
            }
            $data['status'] = 1;
