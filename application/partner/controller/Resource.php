...
class Resource extends Common
{
    ...
    public function index()
    {
        if (request()->isAjax()) {
            ...
            foreach ($list['data'] as $k => $v) {
                ...
                $list['data'][$k]['auth'] = 'off';
                if (in_array($v['id'], $spread)) {
                    $list['data'][$k]['spread'] = 1;
                }
                if ($v['admin_id'] == $this->admin_uid) {
                    $list['data'][$k]['auth'] = 'on';
                }
    ...
    public function spread()
    {
        if (request()->isAjax()) {
            ...
            foreach ($resources as $resource) {
                #主站的卡密资源=代理不可推广
                if($resource->type==6 && $resource->admin_id!==$this->admin_uid){
                    continue;
                }
                ...
                if ($spreadInfo) {//更新
                    ...
                    $resource_arr['price'] = ($agent_price_type == 1) ? $this->auth->min_pub : $resource->price;
                    $resource_arr['dis_price'] = ($agent_price_type == 1) ? $this->auth->dis_pub : $resource->dis_price;
    ...
    public function kammi()
    {
        ...
        if (request()->isAjax()) {
            $list = ResourceKammi::alias('a')->field('a.*,b.nickname')->where('a.rid', $data['ids'])->join('user b', 'a.uid=b.id', 'left');
    ...
    public function course()
    {
        ...
        if (request()->isAjax()) {
            ...
            $list = $db->where('rid', $data['ids'])->order('indexid desc')->paginate(['list_rows' => $data['limit'], 'page' => $data['page']])->toArray();
            return json(["code" => 0, "count" => $list['total'], 'data' => $list['data']]);
        } else {
            $this->assign('res', $res);
            return view();
