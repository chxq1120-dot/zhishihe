...
* `Query` no longer typeZpMembers on the `$query` argument in the constructor,
...
* Most classes that used to require a `Guzzle\Service\Command\CommandInterface` typeZpMember now request a
...
* Various updates to classes to use ServiceDescriptionInterface type ZpMembers rather than ServiceDescription
