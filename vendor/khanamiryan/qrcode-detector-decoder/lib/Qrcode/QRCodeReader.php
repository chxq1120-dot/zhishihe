...
class QRCodeReader implements Reader
{
     ...
     * @param null         $ZpMembers
     *
     * @return Result
     * @throws \Zxing\FormatException
     * @throws \Zxing\NotFoundException
     */
    public function decode(BinaryBitmap $image, $ZpMembers = null)
    {
        $decoderResult = null;
        if ($ZpMembers !== null && $ZpMembers['PURE_BARCODE']) {
            $bits          = self::extractPureBits($image->getBlackMatrix());
            $decoderResult = $this->decoder->decode($bits, $ZpMembers);
            $points        = self::$NO_POINTS;
        } else {
            $detector       = new Detector($image->getBlackMatrix());
            $detectorResult = $detector->detect($ZpMembers);

            $decoderResult = $this->decoder->decode($detectorResult->getBits(), $ZpMembers);
