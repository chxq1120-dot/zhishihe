...
class Tree
{
     ...
     * @return Tree
     */
    public function init($arr = [], $pidname = null, $nbsp = null, $childname = null)
    {
        $this->arr = $arr;
        if (!is_null($pidname)) {
    ...
    public function getTreeUl($myid, $itemtpl, $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '')
    {
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $str .= strtr($nstr, array('@' . $this->childlist => $childlist));
            }
        }
        return $str;
    }
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $value = array(
                    ...
                    '@url' => $childdata || !isset($value['@url']) ? "javascript:;" : createUrl($value['@url']),
                    ...
                    '@class' => ($selected ? ' layui-this' : '') . ($disabled ? ' disabled' : '') . ($childdata || $value['@pid']==0 ? ' layui-nav-item' : ''),
                    '@layhref' => $childdata ? '' : 'lay-href="' . createUrl($value['@url']) . '"',
                    '@laydirection'=>$value['@pid']==0 ? 'lay-direction="2"' : '',
                );
    ...
    public function getTreeArray($myid, $itemprefix = '')
    {
        ...
        if ($childs) {
            ...
            foreach ($childs as $id => $value) {
                ...
                $value['spacer'] = $spacer;
                $data[$n] = $value;
                $data[$n][$this->childlist] = $this->getTreeArray($id, $itemprefix . $k . $this->nbsp);
                $n++;
                $number++;
            }
        }
    ...
    public function getTreeList($data = [], $field = 'name')
    {
        $arr = [];
        foreach ($data as $k => $v) {
            $childlist = isset($v[$this->childlist]) ? $v[$this->childlist] : [];
            unset($v[$this->childlist]);
     ...
     * @return array
     */
    public function getCityList($data = [], $pid = 0)
    {
        $arr = [];
        foreach ($data as $k => $v) {
            if ($v['pid'] == $pid) {
                $arr[$v['id']] = ['id' => $v['code'], 'name' => $v['name']];
                unset($data[$k]);
                $arr[$v['id']][$this->childlist] = $this->getCityList($data, $v['code']);
            }
        }
        return $arr;
    }
